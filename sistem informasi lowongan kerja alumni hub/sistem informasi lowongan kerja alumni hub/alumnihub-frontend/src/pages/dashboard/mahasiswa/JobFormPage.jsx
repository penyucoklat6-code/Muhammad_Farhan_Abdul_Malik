...
const JobFormPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [catRes, majRes, mitraRes] = await Promise.all([
          api.get('/categories'), api.get('/majors'), api.get('/mitras')
        ]);
        ...
        if (isEdit) {
          ...
          setFormData({
            ...
            type: job.type || 'full-time', work_arrangement: job.work_arrangement || 'WFO',
            category_id: job.category_id || '', major_id: job.major_id || '',
            description: job.description || '', requirements: job.requirements || '',
            salary_min: job.salary_min || '', salary_max: job.salary_max || '',
            deadline: job.deadline ? job.deadline.split('T')[0] : '', external_link: job.external_link || ''
          });
        }
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.salary_min && formData.salary_max && Number(formData.salary_min) > Number(formData.salary_max)) {
      setError('Gaji minimum harus lebih kecil dari gaji maksimum.'); setLoading(false); return;
    }
    ...
    try {
      ...
      else { await api.post('/jobs', payload); alert('Lowongan berhasil dibuat dan menunggu persetujuan Kaprodi!'); }
      navigate('/dashboard/mahasiswa/jobs');
    } catch (err) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
      <div className="d-flex align-items-center gap-3 p-4" style={{ borderBottom: '1px solid var(--border)' }}>
        <Link to="/dashboard/mahasiswa/jobs" className="btn btn-sm d-flex align-items-center justify-content-center" style={{ width: '36px', height: '36px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', color: 'var(--text-muted)', background: 'var(--bg-card)' }}>
          <ArrowLeft size={16} />
        </Link>
        <h5 className="mb-0 fw-bold" style={{ color: 'var(--text-main)' }}>{isEdit ? 'Edit Lowongan' : 'Buat Lowongan Baru'}</h5>
      </div>
      ...
      <div className="p-4">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="row mb-4">
            ...
            <div className="col-md-3 mb-3">
              ...
              <select name="type" className="form-select" style={inputStyle} value={formData.type} onChange={handleChange} required>
                ...
                <option value="internship">Internship</option><option value="contract">Contract</option><option value="freelance">Freelance</option>
              </select>
            </div>
            <div className="col-md-3 mb-3">
          ...
          <div className="d-flex justify-content-end gap-2 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
            <Link to="/dashboard/mahasiswa/jobs" className="btn px-4" style={{ background: 'var(--light)', color: 'var(--text-main)', borderRadius: 'var(--radius-sm)' }}>Batal</Link>
            <button type="submit" className="btn btn-primary px-4 d-flex align-items-center gap-2" disabled={loading}>
              <Save size={18} /> {loading ? 'Menyimpan...' : 'Simpan Lowongan'}
            </button>
          </div>
        </form>
      </div>
